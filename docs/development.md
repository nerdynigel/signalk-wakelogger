...
Release automation currently accepts beta tags only and publishes them under npm's `beta` dist-tag. Until a stable release exists, Signal K App Store discovery also requires the package owner to move npm's `latest` alias to the vetted beta interactively after publication. A stable release workflow will be added only after its acceptance gate is agreed. The package-name bootstrap, OIDC and App Store alias procedure are in [release.md](release.md). See [the original-plan gap audit](original-plan-gap-audit.md) before claiming first-vessel or stable-production readiness.
